...
\[ (P_Nx)_e  = P_{j_e}x/\mu_{j_e} ,\]
where $j_e$ is the sinogram-bin where event $e$ was detected and $\mu_{j_e}$ is the total number of events in list $N$ detected in the bin $j_e$. 

Using the list-mode forward operator $P_N$, and taking special care of sinogram bins where no events where detected, we equivalently reformulate problem \eqref{eq:main_minimization_problem} in terms of the list-mode operator $P_N$ and list mode data, and we propose the list-mode SPDHG (LM-SPDHG) as shown
in Algorithm~\ref{alg:lmspdhg} for it's numerical solution.
