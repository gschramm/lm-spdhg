...
As indicated by the name, emission data in list-mode format is a chronological list $N$ of detected 
...
To process the list-mode data during reconstruction without binning it into a sinogram,
we introduce the list-mode forward operator $P_N$ mapping the image data $x$ to a 
...
Re-writing the gradient of the negative Poisson loglikihood using list-mode data and the
list-mode forward and adjoint model is straight forward such that any gradient-based
...
Our proposed list-mode SPDHG algorithm (LM-SPDHG) is shown in algorithm~\ref{alg:lmspdhg}.
In contrast to the original SPDHG using binned data (sinograms), the forward and adjoint
PET models have been replaced by their list-mode equivilants as defined above (lines 11 and 13).
...
Moreover, it is in the similar order of the time needed to unlist the list-mode data into
a sinogram which is a pre-requisit and pre-processing overhead for SPDHG.

Another difference compared to SPDHG is the fact that we split the data into subsets by
assigning every $n$-th event of the event list $N$ into $n$ sub list as usually done in list-mode
OSEM.
Compared to the full list-mode forward operator $P^{LM}_N$, we can think of the subset forwad
operator as the same operator with sensivity reduced by a factor of $n$.
In contrast to SPDHG where ever subset usually only contains counts from a subset of sinogram
bins (e.g. from a subset of all acquired views), every subset in LM-SPDHG contains a subset of 
events possibly originating from all bins $i$. 
