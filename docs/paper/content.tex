...
%%%% Discussion on local beta (how to include) + cite Ahn + Tsai

Last but not least, we would like to mention that at a first glance, the  EM-TV algorithms
seems to be an attractive algorithm to solve the TOF PET reconstruction problem with
non-smooth priors.
Note, however, that according to our experience, EM-TV does not converge when the EM step
is performed using conventional ordered subsets which is demonstrated in supplementary Fig.~XXX.
Nevertheless, EM-TV shows fast convergence in the early iterations, especially when using
a moderate amount of subsets which makes EM-TV a very usefule algorithm to calculate the warm
start for (LM)-SPDHG.
