...
Combining gradient operator for $K$ with the mixed L2-L1 norm for $R$ leads to the well-known 
...
TOF PET systems usually results in extremly long computation times, 
because the evaluation of $P$ and $P^T$
for state-of-the-art TOF PET systems is slow and because several hundreds to thousand of 
updates are needed to obtain reasonable convergence.
...
This problem gets even more severe when aiming for a complete GPU implementation 
...
For shorter frames, as present e.g. in the early phase of dynamic scans or when resperatory
...
At a first glance, EM-TV thus seems to be an appealing algorithm to solve our optimization
problem \eqref{eq:primal} for data in listmode format.
However, in contrast to SPDHG, EM-TV does not have guaranteed converge when using subsets.
...
PET operators have been replaced by their listmode equivilants as defined above (lines 11 and 12).
...
Note that step 5 still requires to calcuate a sinogram back projection of a unity sinogram ($P^T 1$)
...
Hence, we recommend to pre-compute the sensitivity image ($P^T 1$) and to store it in memory.
...
mimick one direct plane of a state-of-the art TOF PET scanner with a ring diameter of 650\,mm
...
we finally simulated and reconstructed 3D TOF PET data based on the 3D XCAT [XXX] phantom 
...
Note that in contrast to our 2D experiements, the cost function in the first 3 iterations after
...
Note that in our proof-of-concept implementation, the effetive speed up is less than a 
...
the time needed to calculate the actial TOF projections.
...
% - can be also used for smooth priors, mention SAG, SAGA
% - guaranteed convergence vs EM-TV
%
%
%
%
