...
In general, we can formulate the resulting optimization problem for any \added[id=MH]{PET} imaging system where the
...
problem involving only the listmode forward operator that is of the same form as \eqref{eq:primal}\added[id=GS]{, see appendix \ref{appendix:lm}}. 
...
\added[id=GS]{Last but not least, we also reconstructed a real TOF PET data set of the NEMA image 
